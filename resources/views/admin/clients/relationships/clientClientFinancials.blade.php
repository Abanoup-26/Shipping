...
<div class="card ">
    ...
    <div class="card-body bg-dark bg-gradient">
        <div class="table-responsive">
            <table
                class=" table table-warning table-bordered table-striped table-hover datatable datatable-clientClientFinancials">
                <thead>
                    <tr>
                        <th width="10">
                            ...
                            <td>
                                @can('client_financial_show')
                                    <a class="btn btn-xs btn-primary"
                                        href="{{ route('admin.client-financials.show', $clientFinancial->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                ...
                                @can('client_financial_edit')
                                    <a class="btn btn-xs btn-info"
                                        href="{{ route('admin.client-financials.edit', $clientFinancial->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('client_financial_delete')
                                    <form action="{{ route('admin.client-financials.destroy', $clientFinancial->id) }}"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-clientClientFinancials:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
