                ...
                <div class="container border border-warning p-5">
                    <div class="row">
                        <div class="form-group col-4">
                            <label
                                class="text-warning text-bold fs-4 required">{{ trans('cruds.order.fields.shipment_company') }}</label>
                            <select
                                ...
                                @foreach (App\Models\Order::SHIPMENT_COMPANY_SELECT as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('shipment_company', $order->shipment_company) === (string) $key ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-4">
                            ...
                            <input class="bg-dark text-white form-control {{ $errors->has('weight') ? 'is-invalid' : '' }}"
                                type="text" name="weight" id="weight" value="{{ old('weight', $order->weight) }}">
                    ...
                    <div class="row">
                        <div class="form-group col-6 ">
                            <label class="text-warning text-bold fs-4 required"
                            ...
                            <textarea class="form-control bg-dark text-white ckeditor {{ $errors->has('from') ? 'is-invalid' : '' }}"
                                name="from" id="from">{!! old('from', $order->from) !!}</textarea>
                            @if ($errors->has('from'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('from') }}
                                </div>
                    ...
                    <div class="row">
                        <div class="form-group col-4">
                            ...
                            <input
                                ...
                                type="text" name="chargeable" id="chargeable"
                                value="{{ old('chargeable', $order->chargeable) }}">
                        ...
                        <div class="form-group col-4">
                            ...
                            <input class="form-control bg-dark text-white {{ $errors->has('pieces') ? 'is-invalid' : '' }}"
                                type="number" name="pieces" id="pieces" value="{{ old('pieces', $order->pieces) }}"
                                step="1">
                    ...
                    <div class="row">
                        <div class="form-group col-4">
                            ...
                            <input
                                ...
                                value="{{ old('cash_on_delivery', $order->cash_on_delivery) }}" step="0.01">
                        ...
                        <div class="form-group col-4">
                            ...
                            <textarea class="form-control bg-dark text-white {{ $errors->has('description') ? 'is-invalid' : '' }}"
                                name="description" id="description">{{ old('description', $order->description) }}</textarea>
                        ...
                        <div class="form-group col-4">
                            ...
                            <input
                                ...
                                value="{{ old('custom_value', $order->custom_value) }}" step="0.01">
                    ...
                    <div class="row justify-content-center">
                        <div class="form-group col-6">
                            <label
                                class="form-label text-warning text-bold fs-4">{{ trans('cruds.order.fields.delivery_status') }}</label>
                            <select
                                class="form-control bg-dark text-white {{ $errors->has('delivery_status') ? 'is-invalid' : '' }}"
                                name="delivery_status" id="delivery_status">
                                <option value disabled {{ old('delivery_status', null) === null ? 'selected' : '' }}>
                                    {{ trans('global.pleaseSelect') }}</option>
                                @foreach (App\Models\Order::DELIVERY_STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('delivery_status', $order->delivery_status) === (string) $key ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                            ...
                            return loader.file
                                .then(function(file) {
                                    return new Promise(function(resolve, reject) {
                                        ...
                                        xhr.open('POST',
                                            '{{ route('admin.orders.storeCKEditorImages') }}',
                                            true);
